...
export default function CreatePagamentoModal({ open, setOpen, segurados, apolices }: any) {
    ...
    const sugestoes = (() => {
        ...
        return (segurados ?? []).filter((s: any) => {
            ...
            const cpfBate = numerosTermo.length > 0 && apenasNumeros(s.cpf_cnpj ?? '').includes(numerosTermo);
                ...
                <div className="flex flex-col gap-4 py-2">
                    ...
                    <div className="flex flex-col gap-1" ref={wrapperRef}>
                        ...
                        <div className="relative">
                            ...
                            {sugestoesAbertas && sugestoes.length > 0 && !data.segurado_id && (
                                <div className="absolute left-0 right-0 mt-1 rounded-md border border-sidebar-border bg-background shadow-lg z-50 overflow-hidden py-1 max-h-56 overflow-y-auto">
                                    {sugestoes.map((s: any) => (
                                        <button
                                            key={s.id}
                                            type="button"
                                            onClick={() => selecionarSegurado(s)}
                                            className="flex w-full flex-col items-start px-3 py-2 text-sm hover:bg-muted transition-colors text-left"
                                        >
                                            <span className="font-medium text-foreground">{s.nome_completo}</span>
                                            <span className="text-xs text-muted-foreground">{s.cpf_cnpj}</span>
                                        </button>
                                    ))}
                                </div>
                            )}
                            ...
                            {sugestoesAbertas && buscaSegurado.trim() && sugestoes.length === 0 && !data.segurado_id && (
                                <div className="absolute left-0 right-0 mt-1 rounded-md border border-sidebar-border bg-background shadow-lg z-50 px-3 py-2 text-sm text-muted-foreground">
                        ...
                        {data.segurado_id && (
                            <p className="text-sm text-green-600">✓ Segurado selecionado</p>
                        )}
                        {(errors as any).segurado_id && (
                            <p className="text-sm text-red-500">{(errors as any).segurado_id}</p>
                        )}
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1">
                            ...
                            >
                                <SelectTrigger>
                                    <SelectValue placeholder={data.segurado_id ? "Selecione" : "Escolha um segurado"} />
                                </SelectTrigger>
                                <SelectContent>
                            ...
                            {(errors as any).apolice_id && (
                                <p className="text-sm text-red-500">{(errors as any).apolice_id}</p>
                            )}
                        </div>
                        <div className="flex flex-col gap-1">
                            <p className="text-sm font-medium">Parcela *</p>
                            <Select value={data.parcela} onValueChange={(v) => setData('parcela', v)}>
                            ...
                            {(errors as any).parcela && (
                                <p className="text-sm text-red-500">{(errors as any).parcela}</p>
                            )}
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        <div className="flex flex-col gap-1">
                            ...
                            />
                            {(errors as any).valor && (
                                <p className="text-sm text-red-500">{(errors as any).valor}</p>
                            )}
                        </div>
                        <div className="flex flex-col gap-1">
                            ...
                            {(errors as any).data_pagamento && (
                                <p className="text-sm text-red-500">{(errors as any).data_pagamento}</p>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        {(errors as any).forma_pagamento && (
                            <p className="text-sm text-red-500">{(errors as any).forma_pagamento}</p>
                        )}
                    </div>
                    ...
                    <div className="flex flex-col gap-1">
                        ...
                        {(errors as any).status && (
                            <p className="text-sm text-red-500">{(errors as any).status}</p>
                        )}
                    </div>
